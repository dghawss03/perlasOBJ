...
const companyVideoUrl = 'https://www.youtube.com/results?search_query=Perlas+Objektbetreuung'
const slides = [
  { kicker:'Perlas Objektbetreuung', title:'Objekte in besten Händen.', text:'Perlas steht für zuverlässige Objektbetreuung, persönliche Ansprechpartner und professionell organisierte Dienstleistungen rund um Immobilien und Gewerbeobjekte.', cta:'Perlas kennenlernen', href:'/ueber-uns', image:'/images/perlas-team-concept.png', alt:'Mitarbeiter von Perlas vor einem Firmenfahrzeug', video:true },
  { kicker:'Leistungen von Perlas', title:'Alles für Ihr Objekt.', text:'Von der laufenden Objektbetreuung über Reinigung und Außenpflege bis zu Facility Services. Sie erhalten zahlreiche Leistungen aus einer Hand.', cta:'Leistungen entdecken', href:'/leistungen', image:'/images/perlas-praxis-concept.png', alt:'Facility Team bei der Kontrolle professioneller Reinigungstechnik' },
  { kicker:'Karriere bei Perlas', title:'Gemeinsam mehr bewegen.', text:'Perlas wächst und sucht Menschen, die Verantwortung übernehmen und Teil eines zuverlässigen Teams werden möchten.', cta:'Karriere bei Perlas', href:'/karriere', image:'/images/perlas-team-concept.png', alt:'Kollegiales Facility Team auf einem Gewerbecampus' },
...
export function HeroSlider(){
 ...
 return <section className={`hero-slider ${paused?'is-paused':''}`} id="top" aria-label="Perlas Themen" onTouchStart={e=>touchStart.current=e.touches[0].clientX} onTouchEnd={e=>finishSwipe(e.changedTouches[0].clientX)}>
  <div className="hero-slides">{slides.map((s,i)=>{const Title=i===0?'h1':'h2';return <article key={s.kicker} className={`hero-slide ${i===active?'active':''}`} aria-hidden={i!==active}><div className="hero-panel"><span className="eyebrow light">{s.kicker}</span><Title>{s.title}</Title><p>{s.text}</p><div className="hero-actions"><Link className="hero-cta" to={s.href} tabIndex={i===active?0:-1}>{s.cta}<ArrowRight/></Link></div></div><div className="hero-visual"><img src={assetUrl(s.image)} alt={s.alt}/>{s.video&&<a className="hero-video-play" href={companyVideoUrl} target="_blank" rel="noreferrer" aria-label="Perlas Unternehmensvideo auf YouTube öffnen" tabIndex={i===active?0:-1}><Play fill="currentColor"/></a>}</div></article>})}</div>
