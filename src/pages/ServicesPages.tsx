...
export function ServiceDetailPage(){
 ...
 return <>
  ...
  <section className={`detail-intro section ${['winterdienst','facility-services'].includes(service.slug)?'detail-intro-spaced':''}`}><div><span className="eyebrow">Leistung im Detail</span><h2>{service.detailHeading}</h2></div><div><p>{service.summary}</p>{service.overview?.map(paragraph=><p key={paragraph}>{paragraph}</p>)}</div></section>
